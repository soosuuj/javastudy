...
public class MaintWrapper {
  ...
  public static void ex01() {  // 메소드 정의 (생성)
    int score = 100;
    
    if(score >= 60) {
      System.out.println("합격");
    }
    // 중괄호는 실행문이 하나인 경우에 생략 가능
    if(score < 60) 
  ...
  /*
   * else if문
   * 1. if 문 이후에 조선식이 필요한 경우 사용할 수 있다.
  ...
  /*
   *  switch문
   *  1. 지정된 표현식의 결과에 따라 분기처리한다.(true/false에 의한 분기처리가 아니다.)
   *  2. 일반적으로 if문에 비해서 가독성이 좋다.(성능x)
