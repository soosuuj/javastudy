 ...
 *  3) 메소드의 반환(return)이 존재하지 않는다. 
 *     반환(return) 반환이 없다는 의미의 void가 아니라 반환(return)이라는 개념 자체가 없다.
 *  4) 파라미터는 일반 메소드와 동일하게 사용할 수 있다. 
 *     메소드 오버로딩(overloading- 같은 이름의 메소드가 여러개 있다 대신 파라미터가 타입, 갯수가 달라야함)이 
 *     가능하다.(생성자를 여러개 만들 수 있다.)
