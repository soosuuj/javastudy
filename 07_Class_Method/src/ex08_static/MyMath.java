 ...
 * 5. 클래스마다 1개씩만 메모리에 로드된다.(모든 인스턴스가 공유할 수 있다.) 100개가 올라가도 하나만 로드? - 싱글턴 패턴에서 사용...ㅎㅎ
 * 6. 클래스를 이용해서 호출한다.(인스턴스를 만든 후 인스턴스로 호출할 수도 있지만 권장하지 않는다. - 실행은 되지만 경고 메세지 뜸)
...
public class MyMath {
  ...
  private static double PI; // static이 붙으면 변수명이 눞는다~ 인스턴스 만들기 전 사용
  
  public MyMath() { // 객체 생성,, 
    PI = 3.14;    // 인스턴스 만든 뒤 값을 줌 -> 아주 잘못된 코드!!!!
  }
*/
